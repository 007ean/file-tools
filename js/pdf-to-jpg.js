(() => {
  ...
  const settingsStore = window.FileTools?.bindToolSettings("pdf-to-jpg", ["scope", "ranges", "scale", "jpgQuality"]);
  ...
  function resetAll(resetSettings) {
    ...
    jpgQuality.value = jpgQuality.value || "0.9";
    jpgQualityVal.textContent = String(Math.round(Number(jpgQuality.value) * 100));
  ...
  async function renderPageToJpg(pdf, pageNumber1Based, scaleFactor, quality, job) {
    ...
    if (!blob) throw new Error("Failed to export JPG for a page.");
    return blob;
  }
  ...
  function addDownloadRow(label, blob, filename) {
    ...
    meta.textContent = `${humanBytes(blob.size)} - image/jpeg`;
  ...
  async function exportJpgs() {
    ...
    const quality = Math.max(0.1, Math.min(1, Number(jpgQuality.value) || 0.9));
    ...
    try {
      ...
      for (let i = 0; i < targets.length; i++) {
        ...
        const blob = await renderPageToJpg(pdf, pageNum, scaleFactor, quality, currentJob);
        const pageSuffix = `page-${String(pageNum).padStart(3, "0")}`;
        const filename = window.FileTools?.makeDownloadName(base, pageSuffix, "jpg")
          || `${base}-${pageSuffix}.jpg`;
      ...
      if (window.JSZip) {
        ...
        zipFilename = window.FileTools?.makeDownloadName(base, "jpg-pages", "zip")
          || `${base}-jpg-pages.zip`;
      ...
      setStatus(zipBlob ? "Done. JPG links and ZIP are ready." : "Done. JPG links are ready.");
    } catch (e) {
      console.error(e);
  ...
  function downloadZip() {
    setError("");
    if (!zipBlob || !zipUrl) {
      setError("No ZIP is ready yet. Export JPGs first.");
      return;
    }
    const a = document.createElement("a");
    a.href = zipUrl;
    a.download = zipFilename || "pdf-jpg-pages.zip";
